import {optimize} from './glslOptimizer';
...
class ShaderLibrary {
    ...
    validateShadersGLSL() {
        slVersions.forEach(slVersion => {
            if (isGLSL[slVersion]) {
                this.optimizedVertexShaders = this.vertexShaders.map(vs => {
                    const result = optimize(vs.generatedSource[slVersion], glslOptimizerTarget[slVersion], 'vs');
                ...
                this.optimizedFragmentShaders = this.fragmentShaders.map(fs => {
                    const result = optimize(vs.generatedSource[slVersion], glslOptimizerTarget[slVersion], 'fs');
                    if (result.type === 'error') {
                        setErrorLocation(vs.filePath, vs.lineNumber);
                        fmtError(result.error);
                    }
...
class Parser {
    ...
    onIn(args) {
        if (!self.current || ['vs', 'fs'].indexOf(this.current.getTag()) === -1) {
    ...
    onUseCodeBlock(args) {
        if (!self.current || ['code_block', 'vs', 'fs'].indexOf(this.current.getTag()) === -1) {
    ...
    onUseTextureBlock(args) {
        ...
        args.forEach(name => {
            ...
            const textureBlock = self.shaderLib.textureBlocks[name];
    ...
    onEnd(args) {
        if (!self.current || ['uniform_block', 'texture_block', 'code_block', 'vs', 'fs', 'program'].indexOf(this.current.getTag()) === -1) {
